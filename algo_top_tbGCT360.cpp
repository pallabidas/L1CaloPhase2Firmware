...
//  GCTcard.RCTcardEtaNeg[2].RCTtoGCTfiber[3].RCTtowers[7].et = 5 ;
  ...
  ap_uint<576> link_in[N_INPUT_LINKS]; // 40 input links, each GCT phi-half (SLR) has 10 RCT cards, each card having 4 output links
  ap_uint<576> link_out[N_OUTPUT_LINKS]; // 24 output links
  ap_uint<32> start = 0;
  ...
  algo_topGCT360(link_in, link_out);
    ...
    #pragma HLS unroll 
        ...
        #pragma HLS unroll 
          ap_uint<32> start = k*17 + 4;
          ap_uint<32> end = k*17 + 16 ;
          cout << " " << link_out[i].range(end,start) ;
        }
        cout << " / " ;
        for(loop k=0; k<N_GCTCLUSTERS_FIBER; k++){
        #pragma HLS UNROLL
            ap_uint<32> start = 289 + k*64 ;
//            ap_uint<32> end =  314 + 60*k;
            ap_uint<32> end =  289 + k*64 + 11;
            cout << " " << link_out[i].range(end, start) ;
        }
//        cout << endl ;
        cout << " / " ;
        ...
        #pragma HLS UNROLL
            ap_uint<32> start = 417 + k*64 ;
            ap_uint<32> end =  417 + k*64 + 11;
            cout << " " << link_out[i].range(end, start) ;
        }
        cout << endl ;
...
//  start = 0;
//  for(loop i = 0; i < 6; i++){
//    cout << "pfcluster " << i << " in positive eta: " << link_out[0].range(start+11, start) << " " << link_out[0].range(start+18, start+12) << " " << link_out[0].range(start+25, start+19) << endl;
//    cout << "pfcluster " << i << " in negative eta: " << link_out[1].range(start+11, start) << " " << link_out[1].range(start+18, start+12) << " " << link_out[1].range(start+25, start+19) << endl;
//    start += 26;
//  }
